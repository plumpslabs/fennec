...
describe("CorrelationEngine", () => {
  ...
  it("should create timeline with trigger and related events", () => {
    ...
    const trigger = makeBusEvent("browser:network", { method: "POST", url: "/api/login", status: 500 });
  ...
  it("should detect root cause for server error pattern", () => {
    ...
    const trigger = makeBusEvent("browser:network", { method: "POST", url: "/api/data", status: 500 });
  ...
  it("should filter by minimum confidence", () => {
    const highConfidenceEngine = new CorrelationEngine(eventBus, { windowMs: 1000, minConfidence: 0.95 });
