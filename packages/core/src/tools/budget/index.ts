...
export const budgetCheckPage = createTool({
  ...
  handler: async (input, { sessionManager, responseBuilder }) => {
    ...
    try {
      ...
      // JS heap budget
      const heapMB = metrics.memoryUsage
        ? Math.round(metrics.memoryUsage.jsHeapSize / (1024 * 1024))
        : null;
      results.push({
        name: 'JS Heap Size',
      ...
      return responseBuilder.error(error, {
        ...
        suggestions: [
          ...
          'Try calling devtools_get_performance_metrics first',
        ],
      });
    }
  },
...
export const budgetGetSummary = createTool({
  ...
  handler: async (input, { sessionManager, responseBuilder }) => {
    ...
    try {
      ...
      const timingBreakdown = await session.browser
        .evaluate(() => {
          ...
          return {
            ...
            secureTime: nav.secureConnectionStart ? nav.connectEnd - nav.secureConnectionStart : 0,
          };
        })
        .catch(() => null);
