...
export const toolsHelp = createTool({
  name: "tools_help",
  category: "ai",
  description: "`<use_case>Discoverability</use_case> 🔎 List available Fennec tools grouped by category, with a one-line purpose each — so you can find the right tool without reading 80+ schemas. Pass category to narrow (e.g. 'auth', 'navigation', 'devtools', 'network', 'process', 'dom', 'smart', 'ai', 'diagnostic'). Omit category to list everything by group. Pairs with the `_tokenTier` tags in tools/list to pick cheap tools first.`",
  ...
  handler: async (input, { responseBuilder, toolRegistry }) => {
    if (!toolRegistry) {
      return responseBuilder.error(
        new Error("Tool registry is not available in this context"),
