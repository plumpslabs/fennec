...
import { readLogLines, redactLine, stripAnsi, parseDuration, redactionCount, classifyLevel } from "../utils/redact.js";
...
export async function inspectCommand(args: string[]): Promise<void> {
  ...
  const tail = tailIndex !== -1 ? Math.min(parseInt(args[tailIndex + 1]!, 10) || DEFAULT_TAIL, MAX_TAIL) : DEFAULT_TAIL;
  ...
  const lines = readLogLines(logPath, { tail: Math.max(tail, sinceMs ? MAX_TAIL : 0), sinceMs: sinceMs ?? undefined, redact: true, parseTimestamp: true });
  ...
  if (proc.port) {
    try { portHealthy = await checkPort(proc.port); } catch { portHealthy = false; }
  }
  ...
  if (plain) {
    ...
    const portStr = proc.port ? ` :${proc.port}${portHealthy === false ? pc.red(" (port down!)") : portHealthy ? pc.green(" (up)") : ""}` : "";
    console.error(`\n  ${symbols.fox} ${renderAppName(proc.name)}${portStr} ${dot} ${running ? pc.green(`running PID ${proc.pid}`) : pc.red("stopped")}`);
    console.error(`  ${pc.dim("uptime")} ${pc.cyan(uptimeSec ? formatUptime(uptimeSec) : "—")}  ${pc.dim("rss")} ${pc.cyan(metrics.rssMb ? `${metrics.rssMb}MB` : "—")}  ${pc.dim("errors(last)")} ${pc.cyan(String(errorLines.length))}  ${pc.dim("redacted")} ${pc.cyan(String(redactedLines))}`);
    if (errorLines.length) {
      console.error(`\n  ${pc.bold(pc.red("Recent errors:"))}`);
      for (const l of errorLines.slice(-8)) console.error(`  ${pc.red(stripAnsi(l).slice(0, 300))}`);
    }
    console.error();
    return;
  }
