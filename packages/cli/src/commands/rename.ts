...
import { renderError, renderKVColor, createSpinner, confirmPrompt, symbols } from "../utils/format.js";
...
export async function renameCommand(args: string[]): Promise<void> {
  ...
  if (isProcessRunning(match.pid)) {
    console.error(`  ${pc.yellow("⚠")} ${pc.dim("Process is still running — log entries will go to the old file until stopped.")}`);
  }
  console.error();

  const confirmed = await confirmPrompt(`Rename ${pc.bold(oldName)} ${pc.dim("→")} ${pc.bold(newName)}?`, true);
