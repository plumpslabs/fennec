 ...
 */
import { existsSync, readFileSync, unlinkSync, statSync, openSync, fstatSync, readSync, closeSync } from "node:fs";
...
import { redactLine, stripAnsi, readLogLines, parseDuration, redactionCount } from "../utils/redact.js";
...
export async function logCommand(args: string[]): Promise<void> {
  ...
  const lineCount = linesIndex !== -1 ? Math.min(parseInt(args[linesIndex + 1]!, 10), MAX_LOG_LINES) : 30;
  ...
  if (jsonFlag) {
    ...
    if (logFilePath && existsSync(logFilePath)) {
      ...
      if (!isNaN(pid)) {
        try {
          const output = execSync(`journalctl --no-pager -n ${Math.max(lineCount, 500)} _PID=${pid} 2>/dev/null || echo ""`, { encoding: "utf-8", timeout: 3000 });
  ...
  if (trackedMatch) {
    ...
    if (!running) {
      console.error(`  ${pc.dim("This app is not running. Showing last captured logs.")} ${pc.cyan(`fennec spawn ${displayName}`)} ${pc.dim("to restart.")}`);
    }
  }
  ...
  const emit = (raw: string): void => {
    ...
    if (opts.json) {
      try {
        ...
        return;
      } catch { /* not valid JSON — fall through to plain print */ }
    }
    const display = line.length > 300 ? line.slice(0, 300) + "…" : line;
    if (line.toLowerCase().includes("error") || line.toLowerCase().includes("fail") || line.toLowerCase().includes("fatal")) {
      console.error(`  ${pc.red(display)}`);
...
function listAvailableApps(): void {
  ...
  if (tracked.length === 0) {
    ...
    console.error(`  ${pc.dim("Start one with:")} ${pc.cyan("fennec start <command> --name <name>")}\n`);
    return;
  }
  ...
  for (const t of tracked) {
    ...
    console.error(`      ${pc.cyan(`fennec log ${t.name}`)} ${pc.dim("·")} ${pc.cyan(`fennec log ${t.name} -f`)}`);
  }
   ...
   console.error(`  ${pc.dim("AI mode:")} ${pc.cyan("fennec log <app> --json --since 10m")} ${pc.dim("— bounded, redacted, machine-readable. Secrets are redacted by default.")}\n`);
}
