...
export async function importCommand(args: string[]): Promise<void> {
  ...
  let imported: { name: string; pid: number; command: string; port?: number; cwd?: string; startedAt: string }[];
  try {
  ...
  saveTracked(merged);
  console.error(`  ${pc.green("✓")} ${pc.bold(`Imported ${imported.length} process(es)`)} ${pc.dim(`(${merged.length} total)`)}\n`);
}
