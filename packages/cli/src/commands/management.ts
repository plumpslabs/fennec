...
import { renderCommand, renderError, renderSuccess, createSpinner, confirmPrompt } from "../utils/format.js";
...
export async function initCommand(): Promise<void> {
  ...
  if (existsSync(configFile)) {
    const overwrite = await confirmPrompt(`${pc.yellow("fennec.config.yaml")} already exists. Overwrite?`, false);
    if (!overwrite) { console.error(`  ${pc.dim("Cancelled.")}\n`); return; }
  }
