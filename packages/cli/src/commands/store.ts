...
export async function storeCommand(args: string[]): Promise<void> {
  ...
  if (kind !== "session") {
    ...
    const tip =
      kind === "process" ? "Managed via `fennec ps` / `fennec kill` / `fennec restart`."
      : kind === "export" ? "Created by `fennec export` / `fennec import`."
  ...
  if (action === "rm" || action === "remove" || action === "delete") {
    ...
    console.error(ok ? `  ${pc.green("✓")} ${pc.bold(name)} deleted\n` : `  ${pc.red("✗")} failed to delete ${pc.bold(name)}\n`);
    return;
  }
