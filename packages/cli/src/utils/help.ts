...
export const COMMANDS: Record<string, CommandDoc> = {
  ...
  db: {
    ...
    options: [
      ['--json', 'JSON output (query)'],
      ['--csv', 'CSV output (query)'],
      ['--no-strict', 'Allow write queries'],
      ['--max-rows N', 'Row limit (default 1000)'],
      ['--no-save', "Don't save credential"],
      ['--database <db>', 'Filter by database (schema/tables)'],
    ],
    examples: [
      'db connect mypg --url "postgres://user:pass@localhost/mydb"',
      'db query mypg "SELECT * FROM users LIMIT 5"',
      'db query mypg "SELECT * FROM users" --json',
      'db schema mypg',
      'db tables mypg',
      'db ps',
      'db update',
  ...
  'db-query': {
    ...
    options: [
      ['--json', 'JSON output'],
      ['--csv', 'CSV output'],
      ['--no-strict', 'Allow write queries (INSERT, UPDATE, DELETE, etc.)'],
      ['--max-rows N', 'Maximum rows to return (default 1000)'],
    ],
    examples: ['db query mypg "SELECT * FROM users"', 'db query mypg "SELECT * FROM users" --json'],
  },
  'db-disconnect': {
    name: 'db disconnect',
  ...
  'db-list': {
    name: 'db list',
    usage: 'db list',
    summary: 'Alias for db ps',
    description: 'Alias for db ps. Shows agent status and active connections.',
    examples: ['db list'],
  },
  'db-schema': {
    name: 'db schema',
  ...
  'db-tables': {
    name: 'db tables',
    usage: 'db tables <name> [--database <db>]',
    summary: 'List tables in a database',
    description: 'Lists all tables and their row counts for the connected database.',
    options: [['--database <db>', 'Filter to a specific database']],
    examples: ['db tables mypg', 'db tables mypg --database mydb'],
  },
  'db-ping': {
    name: 'db ping',
    usage: 'db ping <name>',
    summary: 'Ping a database connection',
    description: 'Checks if a database connection is healthy and reports latency.',
    examples: ['db ping mypg'],
  },
  'db-stats': {
    name: 'db stats',
    usage: 'db stats <name>',
    summary: 'Get database statistics',
    description: 'Reports database size, table count, active connections, and uptime.',
    examples: ['db stats mypg'],
  },
  'db-explain': {
    name: 'db explain',
