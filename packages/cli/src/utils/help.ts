...
export function showHelp(): void {
  ...
  lines.push(`  ${pc.dim("Run")} ${pc.cyan("fennec <command> --help")} ${pc.dim("for details on any command.")}`);
...
export function showCommandHelp(cmd: string): void {
  ...
  if (!doc) {
    ...
    console.error(`  ${pc.dim("Run")} ${pc.cyan("fennec help")} ${pc.dim("to see all commands.")}\n`);
    return;
  }
